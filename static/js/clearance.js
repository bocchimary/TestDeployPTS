...
// Defensive check for clearanceButton
const clearanceBtn = document.getElementById('clearanceButton');
if (clearanceBtn) {
  clearanceBtn.addEventListener('click', function () {
    const canSubmit = this.dataset.canSubmit === 'true';
    if (canSubmit) {
      openClearanceModal();
    } else {
      Swal.fire('Not Allowed', 'You still have an ongoing clearance. Please complete it first.', 'warning');
    }
  });
}